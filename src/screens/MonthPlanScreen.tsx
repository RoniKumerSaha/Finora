...
import { SummaryCell } from '../components/planner/SummaryCell';
import { EmojiPicker } from '../components/planner/EmojiPicker';
import { fillStatus, FILL, formatPct, pctOf, liquidTop, frostedPillStyle } from '../components/planner/jarVisuals';
  ...
  return (
    <div className="flex flex-col gap-5 max-w-5xl w-full">
      ...
      <SaveResetBar
        ...
        onSave={() => savePlan(activeKey)}
      />

      <IncomeField
        value={plan.plannedIncome}
        onCommit={n => patchPlan(activeKey, { plannedIncome: n })}
      />

      <div className="card flex items-center divide-x divide-border">
        <SummaryCell label="Income" value={fmtBDT(summary.plannedIncome)} />
        <SummaryCell
          label="Spent / budgeted"
          value={(() => {
            // Colour the Spent number the same way jar tiles colour:
            ...
            const spentColor = over ? 'var(--danger)' : tight ? 'var(--orange)' : 'var(--accent)';
            return (
              <>
                <span style={{ color: spentColor }}>{fmtBDT(summary.plannedSpend)}</span>{' '}
                <span className="text-muted">/ {fmtBDT(summary.totalBudget)}</span>
              </>
            );
          })()}
          centered
        ...
        <SummaryCell
          label="Saving"
          value={(() => {
            if (summary.saved > 0) return <span className="text-success">{fmtBDT(summary.saved)}</span>;
            if (summary.deficit) return <span className="text-danger">Over income</span>;
...
function IncomeField({ value, onCommit }: { value: number; onCommit: (n: number) => void }) {
  // Explicit Save / Discard — same pattern as EventPlanDetailScreen's
  // EventBudgetField. The store only mutates when the user taps ✓;
  // Escape or the × icon reverts.
  ...
  return (
    <div className="card flex flex-wrap items-center gap-3">
      <div className="flex-1 min-w-[200px]">
        <div className="text-[11px] text-muted uppercase tracking-[0.08em] font-semibold">Income</div>
        <div className="font-bold text-[22px] tabular mt-1 flex items-center gap-2">৳&nbsp;
          <input
            type="number"
            inputMode="decimal"
            min={0}
            ...
            placeholder="0"
            aria-label="Income"
            className="bg-transparent border-0 border-b border-dashed border-border focus:border-primary focus:outline-none w-[180px] tabular text-ink"
          />
          {dirty && (
            <div className="flex items-center gap-1">
              <button
                ...
                title="Discard"
                className="w-7 h-7 rounded-full inline-flex items-center justify-center text-muted hover:text-ink hover:bg-surface-2 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
              >
              ...
              <button
                ...
                title="Save"
                className="w-7 h-7 rounded-full inline-flex items-center justify-center text-success hover:text-success hover:bg-success-soft transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
              >
      ...
      <div className="text-xs text-muted whitespace-nowrap">
        ⓘ How much you expect to bring in this month. The <b className="text-ink">Saving</b> summary below = Income − Spent.
      </div>
    </div>
  );
}
...
function JarTile({ cat, selected, onSelect }: { cat: PlanCategory; selected: boolean; onSelect: () => void }) {
  ...
  const status = fillStatus(pct, overflow, budget);
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{
          top: `${fillTop}%`,
          background: `linear-gradient(180deg, ${FILL[status].soft} 0%, ${FILL[status].color} 30%, ${FILL[status].color} 100%)`,
      ...
      <span
        ...
        style={{
          background: FILL[status].color,
          boxShadow: status === 'empty' ? 'none' : `0 0 0 3px color-mix(in srgb, ${FILL[status].color} 25%, transparent)`,
        }}
      />
        ...
        >
          ...
          <div className="flex items-center gap-1.5 text-[10.5px] font-bold leading-none text-ink whitespace-nowrap">
            <span
              ...
              style={{ background: FILL[status].color }}
            />
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3 mb-2 pr-8">
          ...
          >
            <span aria-hidden className="w-1.5 h-1.5 rounded-full" style={{ background: FILL[status].color }} />
          ...
          >
            <span
              ...
              style={{
                ...
                background: `linear-gradient(180deg, ${FILL[status].soft} 0%, ${FILL[status].color} 100%)`,
                opacity: status === 'empty' ? 0 : 1,
              }}
            />
